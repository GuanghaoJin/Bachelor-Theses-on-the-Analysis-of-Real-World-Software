...
bool safe_strtoul(const char *str, uint32_t *out) {
    ...
    *out = 0;
    ...
    if ((errno == ERANGE) || (str == endptr) || (l > UINT32_MAX)) {
        return false;
    }
...
int main() {
    ...
    for (int i = 0; i < sizeof(test_str) - 1; i++) {
        test_str[i] = VERIFIER_nondet_char();
    }

    test_str[sizeof(test_str) - 1] = '\0';

    __CPROVER_assume(test_str != NULL && strspn(test_str, "0123456789") == strlen);
    assert(safe_strtoul(test_str, &result) == true);

    __CPROVER_assume(test_str != NULL && strcspn(test_str, "0123456789") > 0);
    assert(safe_strtoul(test_str, &result) == false);

    __CPROVER_assume(test_str != NULL && test_str[0] == '\0');
    assert(safe_strtoul(test_str, &result) == false);
