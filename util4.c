...
bool safe_cmp(const void *a, const void *b, size_t len) {
...
extern unsigned int VERIFIER_nondet_uint(void);
extern char VERIFIER_nondet_char(void);



int main() {
    int strlen1 = VERIFIER_nondet_uint();
    if (strlen1 < 1 || strlen1 > 1000) {
        abort();
    }
    char *test_str1 = malloc(sizeof(char) * (strlen1 + 1));
    if (!test_str1) {
        return 1;
    }

    for (int i = 0; i < strlen1 - 1; i++) {
        test_str1[i] = VERIFIER_nondet_char();
    }
    test_str1[strlen1 - 1] = '\0';

    int strlen2 = VERIFIER_nondet_uint();
    if (strlen2 < 1 || strlen2 > 1000) {
        free(test_str1);
        abort();
    }
    char *test_str2 = malloc(sizeof(char) * (strlen2 + 1));
    if (!test_str2) {
        free(test_str1);
        return 1;
    }
    ...
    for (int i = 0; i < strlen2 - 1; i++) {
        test_str2[i] = VERIFIER_nondet_char();
    }
    test_str2[strlen2 - 1] = '\0';
    ...
    size_t len_a = strlen(test_str1);
    size_t len_b = strlen(test_str2);
    size_t len = len_a < len_b ? len_a : len_b;

    bool result = safe_memcmp(test_str1, test_str2, len);

    __CPROVER_assert(result == (strncmp(test_str1, test_str2, len) == 0), "safe_memcmp should return true if strings are equal in the given length");
